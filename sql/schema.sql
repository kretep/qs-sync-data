...
--
-- Name: aggregate_by_period(text, text, text, text, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.aggregate_by_period(p_table_name text, p_date_column text, p_agg_column text, p_aggregation_type text, p_period text) RETURNS TABLE(period_start timestamp with time zone, aggregated_value numeric)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY EXECUTE FORMAT('
        SELECT date_trunc(''%I'', %I::TIMESTAMP WITH TIME ZONE) AS period_start,
               CASE WHEN ''%s'' = ''COUNT'' THEN COUNT(%I)::NUMERIC
                    ELSE %s(%I)
               END AS aggregated_value
        FROM %I
        GROUP BY date_trunc(''%I'', %I::TIMESTAMP WITH TIME ZONE)
        ORDER BY period_start
    ', p_period, p_date_column, p_aggregation_type, p_agg_column, p_aggregation_type, p_agg_column, p_table_name, p_period, p_date_column);

END;
$$;


ALTER FUNCTION public.aggregate_by_period(p_table_name text, p_date_column text, p_agg_column text, p_aggregation_type text, p_period text) OWNER TO postgres;

--
-- Name: calculate_sum_by_week(); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.calculate_sum_by_week()
    LANGUAGE sql
    AS $_$CREATE OR REPLACE FUNCTION calculate_sum_by_week(
    p_table_name TEXT,
    p_date_column TEXT,
    p_sum_column TEXT
)
RETURNS TABLE (week_start DATE, total_sum NUMERIC) AS $$
BEGIN
    RETURN QUERY EXECUTE FORMAT('
        SELECT date_trunc(''week'', %I::DATE) AS week_start,
               SUM(%I) AS total_sum
        FROM %I
        GROUP BY EXTRACT(YEAR FROM %I::DATE), date_trunc(''week'', %I::DATE)
        ORDER BY EXTRACT(YEAR FROM %I::DATE), week_start
    ', p_date_column, p_sum_column, p_table_name, p_date_column, p_date_column, p_date_column);
END;
$$ LANGUAGE plpgsql;
$_$;


ALTER PROCEDURE public.calculate_sum_by_week() OWNER TO postgres;
...
--
-- Name: clippings; Type: TABLE; Schema: public; Owner: postgres
--
...
ALTER TABLE public.clippings OWNER TO postgres;

--
-- Name: hardlooptijden; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.hardlooptijden (
    date date,
...
ALTER TABLE public.hardlooptijden OWNER TO postgres;

--
-- Name: meterstanden; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.meterstanden (
    ...
    active_power_w integer NOT NULL,
    total_gas_m3 real NOT NULL,
    gas_timestamp bigint NOT NULL
);


ALTER TABLE public.meterstanden OWNER TO postgres;

--
-- Name: meterstanden_totals; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.meterstanden_totals AS
 SELECT (meterstanden.total_power_import_t1_kwh + meterstanden.total_power_import_t2_kwh) AS total_power_import,
    meterstanden.total_gas_m3,
    meterstanden.created_at
   FROM public.meterstanden;


ALTER TABLE public.meterstanden_totals OWNER TO postgres;

--
-- Name: meterstanden_daily; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.meterstanden_daily AS
 SELECT date(meterstanden_totals.created_at) AS date,
    min(meterstanden_totals.total_power_import) AS total_power_import,
    (min(meterstanden_totals.total_power_import) - lag(min(meterstanden_totals.total_power_import)) OVER (ORDER BY (date(meterstanden_totals.created_at)))) AS delta_power,
    min(meterstanden_totals.total_gas_m3) AS total_gas,
    (min(meterstanden_totals.total_gas_m3) - lag(min(meterstanden_totals.total_gas_m3)) OVER (ORDER BY (date(meterstanden_totals.created_at)))) AS delta_gas
   FROM public.meterstanden_totals
  GROUP BY (date(meterstanden_totals.created_at))
  ORDER BY (date(meterstanden_totals.created_at)) DESC;


ALTER TABLE public.meterstanden_daily OWNER TO postgres;

--
-- Name: random_clipping; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.random_clipping AS
 SELECT clippings.title,
    clippings.author,
    clippings.created,
    clippings.content
...
ALTER TABLE public.random_clipping OWNER TO postgres;

--
-- Name: weerlive; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.weerlive (
    temp real,
...
ALTER TABLE public.weerlive OWNER TO postgres;
...
ALTER TABLE ONLY public.birthdays
    ADD CONSTRAINT birthdays_pkey PRIMARY KEY (name);


--
-- Name: meterstanden meterstanden_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.meterstanden
    ADD CONSTRAINT meterstanden_pkey PRIMARY KEY (created_at);


--
-- Name: weerlive weerlive_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.weerlive
    ADD CONSTRAINT weerlive_pkey PRIMARY KEY (datetime);
